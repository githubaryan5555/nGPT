...
class Config:
    ...
    def __post_init__(self):
        ...
        if self.num_effective_blocks <= 0:
            raise ValueError(
                "num_prefix_blocks + num_shared_blocks + num_suffix_blocks "
                "must be > 0"
            )

        if self.num_shared_blocks <= 0 and (
            self.num_prefix_blocks == 0 or self.num_suffix_blocks == 0
        ):
            raise ValueError(
                "If prefix or suffix blocks are zero, num_shared_blocks "
                "must be > 0"
            )
    ...
    @property
    def num_effective_blocks(self):
        return (
            self.num_prefix_blocks
            + self.num_shared_blocks
            + self.num_suffix_blocks
        )

    @property
    def num_unique_blocks(self):
        return (
            self.num_prefix_blocks
            + (1 if self.num_shared_blocks > 0 else 0)
            + self.num_suffix_blocks
        )
...
class RMSNorm(nn.Module):
    ...
    def forward(self, x):
        ...
        return (
            x
            * torch.rsqrt(variance + self.eps).to(x.dtype)
            * self.weight
        )
...
class RoPE(nn.Module):
    ...
    def forward(
        ...
        if position_offset < 0 or end > self.cos.size(0):
            raise ValueError(
                f"position range [{position_offset}, {end}) exceeds "
...
class Model5555LM(nn.Module):
    """
    Causal language model with hybrid recurrent depth.

    The architecture consists of:

        prefix blocks
            ↓
        one shared block repeated multiple times
            ↓
        suffix blocks
    ...
    def __init__(
        ...
        **overrides,
        ...
        # Unique blocks before the shared block.
        ...
        # It is reused num_shared_blocks times.
        self.shared_block = (
            Block(config)
            if config.num_shared_blocks > 0
            else None
        )

        # Unique blocks after the shared block.
        ...
        residual_std = (
            ...
            / math.sqrt(config.num_effective_blocks)
        )

        for layer in self.prefix_blocks:
            nn.init.normal_(
        ...
        if self.shared_block is not None:
            nn.init.normal_(
                self.shared_block.self_attn.o_proj.weight,
                std=residual_std,
            )
            nn.init.normal_(
                self.shared_block.mlp.down_proj.weight,
                std=residual_std,
            )

        for layer in self.suffix_blocks:
            nn.init.normal_(
                layer.self_attn.o_proj.weight,
                std=residual_std,
            )
    ...
    def forward(
        ...
        if (
            input_ids.min() < 0
            or input_ids.max() >= self.config.vocab_size
        ):
            raise ValueError(
        ...
        # -----------------------------
        # Middle: ONE shared block
        # reused multiple times
        # -----------------------------
        if self.shared_block is not None:
            for _ in range(
                self.config.num_shared_blocks
            ):
                x = self._forward_block(
                    x,
                    self.shared_block,
    ...
    def get_flops_per_token(
        ...
        effective_blocks = (
            self.config.num_effective_blocks
        )
    ...
    def generate(
        ...
        if (
            ids.min() < 0
            or ids.max() >= self.config.vocab_size
        ):
            raise ValueError(
                "tokenizer produced a token outside "
