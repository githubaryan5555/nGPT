...
class TokenBatches:
    def __init__(self):
        ...
        self.use_cuda = device_type == "cuda"

        if self.use_cuda:
            self.cpu_x = torch.empty(
                (batch_size, max_seq_len),
                dtype=torch.long,
                pin_memory=True,
            )
            self.cpu_y = torch.empty(
                (batch_size, max_seq_len),
                dtype=torch.long,
                pin_memory=True,
            )
